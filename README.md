...
| 데이터/AI | openpyxl(POS 파싱), Pillow, OpenAI SDK, 자체 통계 예측 엔진 |
...
bossprofit/
  ...
  profit/
    ...
    forecasting/             단계형 예측 엔진(engine.py)·평가(evaluation.py)
    integrations/            KAMIS(kamis.py)·data.go.kr(data_go_kr.py) 클라이언트
    management/commands/      데이터 수집·예측·랭킹·시드 커맨드
...
python manage.py seed_data          # (선택) 데모 메뉴·재료·레시피 적재
...
python manage.py run_market_forecast       # 단계형 예측 실행
python manage.py backtest_market_forecast  # rolling-origin 백테스트
