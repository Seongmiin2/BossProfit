...
```text
매장 데이터 관리        재료·메뉴·레시피·POS 엑셀 판매 데이터 저장/분석
수익성 계산            레시피 원가, 채널별 마진, 메뉴 수익성 신호등
시장 가격 예측          Base → Weather → Residual 단계형 예측 엔진
ML 예측                장기 horizon용 글로벌 LightGBM 분위 모델 + 통계 모델 폴백
추천                   예측 변동률을 BUY/WATCH/AVOID 구매 행동으로 변환
생성형 AI              매장 데이터 근거 Q&A·리포트 요약, 키 없을 때 규칙 기반 폴백
배포                   로컬 실행 환경 제공, 클라우드 미배포
```

예측 영역은 **검증된 성능 수치를 임의로 노출하지 않는 것**을 전제로 합니다. LightGBM은
학습 아티팩트가 있고 해당 horizon에서 사용하는 조건을 만족할 때만 적용되며, 그렇지 않으면
통계 baseline 또는 last-value 기준선으로 자동 폴백합니다.

---

## 목차

- [한눈에 보는 현재 구현 상태](#한눈에-보는-현재-구현-상태)
...
**horizon별 모델 선택(백테스트 근거).** LightGBM을 모든 예측에 무조건 적용하지 않고,
rolling-origin 백테스트와 학습 아티팩트 존재 여부를 기준으로 사용 구간을 제한합니다.
현재 엔진은 **장기 horizon만 LightGBM 후보 경로**로 두고, 단기(1·7일) 또는 아티팩트가
없는 경우에는 통계 모델/last-value 기준선으로 자동 폴백합니다.
...
> 아티팩트가 없으면 통계 모델로 자동 폴백합니다. 모델 성능 수치는 별도 backtest 결과와
> `is_verified` 상태가 확인된 경우에만 사용자에게 노출하는 것을 원칙으로 합니다.
...
흡수했습니다.
→ *배운 점:* rolling-origin 백테스트로 비교해보니 LightGBM은 **만능 모델이 아니라 적용
조건을 가려야 하는 후보 모델**이었습니다. "모델을 넣었다"가 아니라 *"어디서 안전하게 쓸 수
있는지"*를 측정해 **horizon별로 모델을 갈라 쓰는** 게 정답이었습니다.
→ *새로 배운 것:* 분위(quantile) 회귀로 예측구간을 직접 산출하고, **conformal 보정**으로
실측 커버리지를 80% 목표에 맞춰 보정하는 법. 그리고 학습된 모델은 아티팩트로 분리해,
없으면 통계 모델로 자동 폴백하도록 한 **안전한 점진 도입** 패턴.
→ *느낀 점:* 얇은 데이터에선 화려한 모델보다 **"많은 품목을 한 모델로 묶어 서로의 패턴을
빌려 쓰게 하는"** 구조적 선택이 더 중요했다. 동시에 검증되지 않은 개선 폭은 문서나 화면에
확정 수치처럼 쓰지 않는 것이 신뢰를 지키는 방법이었다.
