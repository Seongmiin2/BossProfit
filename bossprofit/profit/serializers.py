...
class RecipeItemWriteSerializer(serializers.Serializer):
    ...
    def validate_ingredient_id(self, value):
        # 재료 존재 검증은 현재 매장 범위로 한정한다.
        store = self.context.get("store")
        qs = Ingredient.objects.filter(ingredient_id=value)
...
class MenuWriteSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        ...
        for item_data in recipe_items_data:
            ingredient = Ingredient.objects.get(
                ingredient_id=item_data['ingredient_id'], store=menu.store
            )
    ...
    def update(self, instance, validated_data):
        ...
        if recipe_items_data is not None:
            ...
            for item_data in recipe_items_data:
                ingredient = Ingredient.objects.get(
                    ingredient_id=item_data['ingredient_id'], store=instance.store
                )
                RecipeItem.objects.create(
                    menu=instance,
                    ingredient=ingredient,
...
class IngredientWriteSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Ingredient
        fields = ['ingredient_id', 'name', 'category', 'purchase_quantity', 'purchase_price', 'unit', 'memo', 'is_supplied']
...
class IngredientPriceHistorySerializer(serializers.ModelSerializer):
    ...
    delta_rate = serializers.SerializerMethodField()

    class Meta:
        model = IngredientPriceHistory
        fields = [
            'id', 'ingredient_id', 'ingredient_name',
            'old_price', 'new_price', 'delta', 'delta_rate',
            'source', 'note', 'created_at',
        ]

    def get_delta_rate(self, obj):
        return round(obj.delta_rate, 2)
