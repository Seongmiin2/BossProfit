...
def recalculate_all(
    ...
    """매장의 활성 메뉴를 재계산하고 Snapshot을 새로 저장.

    이전 스냅샷은 보존(시계열용). store가 주어지면 해당 매장 메뉴만 처리한다.
    """
    if assumption is None:
        assumption = ProfitAssumption.get_active(store=store)

    menus_qs = Menu.objects.filter(is_active=True)
    if store is not None:
        menus_qs = menus_qs.filter(store=store)
    menus = list(menus_qs.prefetch_related("recipe_items__ingredient"))
...
def get_latest_snapshots(store=None) -> list[MenuProfitSnapshot]:
    """각 메뉴별 가장 최근 Snapshot만 반환. store가 주어지면 해당 매장 메뉴만."""
    menus_qs = Menu.objects.filter(is_active=True)
    if store is not None:
        menus_qs = menus_qs.filter(store=store)
    latest = []
    for menu in menus_qs.order_by("menu_id"):
